...
val read_console_input : Unix.file_descr -> LTerm_event.t
...
val get_console_screen_buffer_info : Unix.file_descr -> console_screen_buffer_info
...
val get_console_mode : Unix.file_descr -> console_mode
(** Returns the mode of the given console. *)

val set_console_mode : Unix.file_descr -> console_mode -> unit
(** Sets the mode of the given console. *)

(** {6 Console cursor} *)

val get_console_cursor_info : Unix.file_descr -> int * bool
(** Returns the size and visible status of the cursor on the given
    console. The size is a percentage between 1 and 100. *)

val set_console_cursor_info : Unix.file_descr -> int -> bool -> unit
(** [set_console_cursor_info fd size visible] sets the size and
    visible status of the cursor on the given console. *)

val set_console_cursor_position : Unix.file_descr -> LTerm_geom.coord -> unit
(** Move the cursor to the specified location in the screen
    buffer. *)

(** {6 Text attributes} *)

val set_console_text_attribute : Unix.file_descr -> text_attributes -> unit
...
val write_console_output
  ...
  -> LTerm_geom.rect
  -> LTerm_geom.rect
(** [write_console_output fd chars size coord rect] writes the given matrix of characters
    with their attributes on the given console at given position. *)
...
val fill_console_output_character
  ...
  -> LTerm_geom.coord
  -> int
(** [fill_console_output_character fd char count coord] writes [count] times [char]
    starting at [coord] on the given console. *)
