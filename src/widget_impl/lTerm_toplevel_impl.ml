...
class toplevel focused widget = object(self)
  ...
  val mutable push_layer_handler = React.E.never;
  val mutable pop_layer_handler = React.E.never;

  method arm_layer_handlers (push_event : t React.event)
                            (push_handler : t -> unit)
                            (pop_event : unit React.event)
