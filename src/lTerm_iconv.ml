...
let iconv cd ~src ~dst =
  if src.index < 0 || src.index > src.limit || src.limit > String.length src.bytes then invalid_arg "LTerm_iconv.iconv";
  if dst.index < 0 || dst.index > dst.limit || dst.limit > String.length dst.bytes then invalid_arg "LTerm_iconv.iconv";
  iconv cd ~src ~dst
