...
let rec parse_event ?(escape_time = 0.1) cd stream =
  ...
  match byte with
            ...
            | Some byte ->
                match byte with
                  ...
                  | '\x00' .. '\x1b' ->
                      ...
                      return (Lt_event.Key { control = (match code with Char _ -> true | _ -> false); meta = true; shift = false; code })
                  | '\x7f' ->
                      (* Backspace *)
                      lwt () = Lwt_stream.junk stream in
                      return (Lt_event.Key { control = false; meta = true; shift = false; code = Backspace })
                  | '\x00' .. '\x7f' ->
                  ...
                  | byte' ->
                      ...
                      return (Lt_event.Key { control = false; meta = true; shift = false; code = Char code })
      end
    | '\x00' .. '\x1f' ->
        (* Control characters *)
        let code = controls.(Char.code byte) in
        return (Lt_event.Key { control = (match code with Char _ -> true | _ -> false); meta = false; shift = false; code })
    | '\x7f' ->
        (* Backspace *)
    ...
    | _ ->
        (* Encoded characters *)
        lwt code = parse_char cd stream byte in
        return (Lt_event.Key { control = false; meta = false; shift = false; code = Char code })
